...
function renderFirstTime(urlPath) {
  ...
  $.ajax({
    ...
    success: (data => {
      ...
      keywordArray.forEach(value => { product.renderOption(value); });
      product.event1();
      product.event2();
    }),
  });
...
function Product(val) {
  ...
  if (keywordArray.includes(this.keyWords) === false) {
    keywordArray.push(this.keyWords);
  }
}

Product.prototype.render = function () {
  let template = $('#mustacheR').html();
...
Product.prototype.event1 = function () {
  $('#filter').change(function () {
    ...
    let select2 = productArray.filter((n) => n.keyWords === choose);
    $('#showImage').empty('');
    filterArray = select2;
...
$('#showImage').on('click',(event) =>{
  ...
  let newClick = productArray.filter(n => n.filePath === clickEle);
  $('#showImage').empty('');
  newClick[0].render();
